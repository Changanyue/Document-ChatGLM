...
from transformers import MBartConfig, MBartForCausalLM, XLMRobertaTokenizer, GPT2Tokenizer, GPT2Config, \
    LlamaTokenizerFast, LlamaConfig
...
from donut.model import SwinEncoder
from my_models.swin_chatglm.configuration_chatglm import ChatGLMConfig
from my_models.swin_chatglm.modeling_chatglm import ChatGLMForConditionalGeneration
from my_models.swin_chatglm.tokenization_chatglm import ChatGLMTokenizer


class ChatglmDecoderForChatDoc(nn.Module):
    """
    ...
    def __init__(
        ...
        #     "/public/home/xlwang2/codes/Med_Prompts/resources/chinese-llama-alpaca-plus-lora-7b" if not name_or_path else name_or_path
        # )
        ...
        #     "/public/home/xlwang2/codes/Med_Prompts/resources/chinese-llama-alpaca-plus-lora-7b" if not name_or_path else name_or_path
        # )
        self.model_config = ChatGLMConfig.from_pretrained(
            "/public/home/xlwang2/codes/Med_Prompts/models--THUDM--chatglm-6b/snapshots/a8ede826cf1b62bd3c78bdfb3625c7c5d2048fbd" if not name_or_path else name_or_path
        )
        self.model_config.add_cross_attention = True

        # self.model = LlamaForCausalLM(
        #     config=self.model_config
        # )
        self.model = ChatGLMForConditionalGeneration(
            config=self.model_config,
        ).quantize(8).half()
        ...
        # weight init with gpt2
        if not name_or_path:
            model_ = ChatGLMForConditionalGeneration.from_pretrained(
                "/public/home/xlwang2/codes/Med_Prompts/models--THUDM--chatglm-6b/snapshots/a8ede826cf1b62bd3c78bdfb3625c7c5d2048fbd" if not name_or_path else name_or_path,
                config=self.model_config,
            )
    ...
    def prepare_inputs_for_inference(self,
        ...
        outputs = self.model.prepare_inputs_for_generation(
            input_ids,
            past=past,
            past_key_values=past_key_values,
            attention_mask=attention_mask,
        )
        outputs["encoder_hidden_states"] = encoder_outputs.last_hidden_state
        outputs["use_cache"] = use_cache
...
from transformers import BartPretrainedModel, PreTrainedModel
...
class DonutModel(PreTrainedModel):
    ...
    def __init__(self, config: DonutConfig):
        ...
        self.decoder = ChatglmDecoderForChatDoc(
            max_position_embeddings=self.config.max_position_embeddings,
            decoder_layer=self.config.decoder_layer,
            name_or_path=self.config.name_or_path,
        )

        self.visual_mlp = nn.Linear(1024, 4096)
    ...
    def forward(self, image_tensors: torch.Tensor,
        ...
        encoder_outputs = self.visual_mlp(encoder_outputs)
